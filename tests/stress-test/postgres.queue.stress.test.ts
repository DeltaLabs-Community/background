...
import {Pool} from "pg";
import { PostgreSQLJobQueue, PostgreSQLJobStorage } from "../../src";
...
async function collectUserInput() {
    try {
        ...
        const hostAnswer = await askQuestion("Please specify the postgres host (default is localhost): ");
        testConfig.postgresHost = hostAnswer || testConfig.postgresHost;

        const portAnswer = await askQuestion("Please specify the postgres port (default is 5342): ");
        const port = parseInt(portAnswer) || testConfig.postgresPort;
        if (port > 0) {
            testConfig.postgresPort = port;
        }

        const userAnswer = await askQuestion("Please specify the postgres user (default is postgres): ");
        testConfig.postgresUser = userAnswer || testConfig.postgresUser;

        const passwordAnswer = await askQuestion("Please specify the postgres password (default is 12345): ");
        testConfig.postgresPassword = passwordAnswer || testConfig.postgresPassword;

        const databaseAnswer = await askQuestion("Please specify the postgres database (default is postgres): ");
        testConfig.postgresDatabase = databaseAnswer || testConfig.postgresDatabase;
...
async function runStressTest() {
    const pool = new Pool({
        database: testConfig.postgresDatabase,
